...
export const retryApiCall = async (apiCall, maxRetries = 3, delay = 1000) => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`API call attempt ${attempt} failed, retrying in ${waitMs}ms...`);
      // Use a per-attempt constant to avoid unsafe closure references in linting
      const currentWait = waitMs;
